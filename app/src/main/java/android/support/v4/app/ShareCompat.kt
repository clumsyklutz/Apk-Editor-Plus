package android.support.v4.app

import android.app.Activity
import android.content.ComponentName
import android.content.Intent
import android.content.pm.PackageManager
import android.graphics.drawable.Drawable
import android.net.Uri
import android.os.Build
import android.os.Parcelable
import android.support.annotation.StringRes
import android.support.v4.content.IntentCompat
import android.text.Html
import android.text.Spanned
import android.util.Log
import android.view.ActionProvider
import android.view.Menu
import android.view.MenuItem
import android.widget.ShareActionProvider
import java.util.ArrayList

class ShareCompat {
    public static val EXTRA_CALLING_ACTIVITY = "android.support.v4.app.EXTRA_CALLING_ACTIVITY"
    public static val EXTRA_CALLING_PACKAGE = "android.support.v4.app.EXTRA_CALLING_PACKAGE"
    private static val HISTORY_FILENAME_PREFIX = ".sharecompat_"

    class IntentBuilder {
        private Activity mActivity
        private ArrayList mBccAddresses
        private ArrayList mCcAddresses
        private CharSequence mChooserTitle
        private Intent mIntent = Intent().setAction("android.intent.action.SEND")
        private ArrayList mStreams
        private ArrayList mToAddresses

        private constructor(Activity activity) {
            this.mActivity = activity
            this.mIntent.putExtra(ShareCompat.EXTRA_CALLING_PACKAGE, activity.getPackageName())
            this.mIntent.putExtra(ShareCompat.EXTRA_CALLING_ACTIVITY, activity.getComponentName())
            this.mIntent.addFlags(524288)
        }

        private fun combineArrayExtra(String str, ArrayList arrayList) {
            Array<String> stringArrayExtra = this.mIntent.getStringArrayExtra(str)
            Int length = stringArrayExtra != null ? stringArrayExtra.length : 0
            Array<String> strArr = new String[arrayList.size() + length]
            arrayList.toArray(strArr)
            if (stringArrayExtra != null) {
                System.arraycopy(stringArrayExtra, 0, strArr, arrayList.size(), length)
            }
            this.mIntent.putExtra(str, strArr)
        }

        private fun combineArrayExtra(String str, Array<String> strArr) {
            Intent intent = getIntent()
            Array<String> stringArrayExtra = intent.getStringArrayExtra(str)
            Int length = stringArrayExtra != null ? stringArrayExtra.length : 0
            Array<String> strArr2 = new String[strArr.length + length]
            if (stringArrayExtra != null) {
                System.arraycopy(stringArrayExtra, 0, strArr2, 0, length)
            }
            System.arraycopy(strArr, 0, strArr2, length, strArr.length)
            intent.putExtra(str, strArr2)
        }

        fun from(Activity activity) {
            return IntentBuilder(activity)
        }

        fun addEmailBcc(String str) {
            if (this.mBccAddresses == null) {
                this.mBccAddresses = ArrayList()
            }
            this.mBccAddresses.add(str)
            return this
        }

        fun addEmailBcc(Array<String> strArr) {
            combineArrayExtra("android.intent.extra.BCC", strArr)
            return this
        }

        fun addEmailCc(String str) {
            if (this.mCcAddresses == null) {
                this.mCcAddresses = ArrayList()
            }
            this.mCcAddresses.add(str)
            return this
        }

        fun addEmailCc(Array<String> strArr) {
            combineArrayExtra("android.intent.extra.CC", strArr)
            return this
        }

        fun addEmailTo(String str) {
            if (this.mToAddresses == null) {
                this.mToAddresses = ArrayList()
            }
            this.mToAddresses.add(str)
            return this
        }

        fun addEmailTo(Array<String> strArr) {
            combineArrayExtra("android.intent.extra.EMAIL", strArr)
            return this
        }

        fun addStream(Uri uri) {
            Uri uri2 = (Uri) this.mIntent.getParcelableExtra("android.intent.extra.STREAM")
            if (this.mStreams == null && uri2 == null) {
                return setStream(uri)
            }
            if (this.mStreams == null) {
                this.mStreams = ArrayList()
            }
            if (uri2 != null) {
                this.mIntent.removeExtra("android.intent.extra.STREAM")
                this.mStreams.add(uri2)
            }
            this.mStreams.add(uri)
            return this
        }

        fun createChooserIntent() {
            return Intent.createChooser(getIntent(), this.mChooserTitle)
        }

        Activity getActivity() {
            return this.mActivity
        }

        fun getIntent() {
            if (this.mToAddresses != null) {
                combineArrayExtra("android.intent.extra.EMAIL", this.mToAddresses)
                this.mToAddresses = null
            }
            if (this.mCcAddresses != null) {
                combineArrayExtra("android.intent.extra.CC", this.mCcAddresses)
                this.mCcAddresses = null
            }
            if (this.mBccAddresses != null) {
                combineArrayExtra("android.intent.extra.BCC", this.mBccAddresses)
                this.mBccAddresses = null
            }
            Boolean z = this.mStreams != null && this.mStreams.size() > 1
            Boolean zEquals = this.mIntent.getAction().equals("android.intent.action.SEND_MULTIPLE")
            if (!z && zEquals) {
                this.mIntent.setAction("android.intent.action.SEND")
                if (this.mStreams == null || this.mStreams.isEmpty()) {
                    this.mIntent.removeExtra("android.intent.extra.STREAM")
                } else {
                    this.mIntent.putExtra("android.intent.extra.STREAM", (Parcelable) this.mStreams.get(0))
                }
                this.mStreams = null
            }
            if (z && !zEquals) {
                this.mIntent.setAction("android.intent.action.SEND_MULTIPLE")
                if (this.mStreams == null || this.mStreams.isEmpty()) {
                    this.mIntent.removeExtra("android.intent.extra.STREAM")
                } else {
                    this.mIntent.putParcelableArrayListExtra("android.intent.extra.STREAM", this.mStreams)
                }
            }
            return this.mIntent
        }

        fun setChooserTitle(@StringRes Int i) {
            return setChooserTitle(this.mActivity.getText(i))
        }

        fun setChooserTitle(CharSequence charSequence) {
            this.mChooserTitle = charSequence
            return this
        }

        fun setEmailBcc(Array<String> strArr) {
            this.mIntent.putExtra("android.intent.extra.BCC", strArr)
            return this
        }

        fun setEmailCc(Array<String> strArr) {
            this.mIntent.putExtra("android.intent.extra.CC", strArr)
            return this
        }

        fun setEmailTo(Array<String> strArr) {
            if (this.mToAddresses != null) {
                this.mToAddresses = null
            }
            this.mIntent.putExtra("android.intent.extra.EMAIL", strArr)
            return this
        }

        fun setHtmlText(String str) {
            this.mIntent.putExtra(IntentCompat.EXTRA_HTML_TEXT, str)
            if (!this.mIntent.hasExtra("android.intent.extra.TEXT")) {
                setText(Html.fromHtml(str))
            }
            return this
        }

        fun setStream(Uri uri) {
            if (!this.mIntent.getAction().equals("android.intent.action.SEND")) {
                this.mIntent.setAction("android.intent.action.SEND")
            }
            this.mStreams = null
            this.mIntent.putExtra("android.intent.extra.STREAM", uri)
            return this
        }

        fun setSubject(String str) {
            this.mIntent.putExtra("android.intent.extra.SUBJECT", str)
            return this
        }

        fun setText(CharSequence charSequence) {
            this.mIntent.putExtra("android.intent.extra.TEXT", charSequence)
            return this
        }

        fun setType(String str) {
            this.mIntent.setType(str)
            return this
        }

        fun startChooser() {
            this.mActivity.startActivity(createChooserIntent())
        }
    }

    class IntentReader {
        private static val TAG = "IntentReader"
        private Activity mActivity
        private ComponentName mCallingActivity
        private String mCallingPackage
        private Intent mIntent
        private ArrayList mStreams

        private constructor(Activity activity) {
            this.mActivity = activity
            this.mIntent = activity.getIntent()
            this.mCallingPackage = ShareCompat.getCallingPackage(activity)
            this.mCallingActivity = ShareCompat.getCallingActivity(activity)
        }

        fun from(Activity activity) {
            return IntentReader(activity)
        }

        private fun withinStyle(StringBuilder sb, CharSequence charSequence, Int i, Int i2) {
            Int i3 = i
            while (i3 < i2) {
                Char cCharAt = charSequence.charAt(i3)
                if (cCharAt == '<') {
                    sb.append("&lt;")
                } else if (cCharAt == '>') {
                    sb.append("&gt;")
                } else if (cCharAt == '&') {
                    sb.append("&amp;")
                } else if (cCharAt > '~' || cCharAt < ' ') {
                    sb.append("&#" + ((Int) cCharAt) + ";")
                } else if (cCharAt == ' ') {
                    while (i3 + 1 < i2 && charSequence.charAt(i3 + 1) == ' ') {
                        sb.append("&nbsp;")
                        i3++
                    }
                    sb.append(' ')
                } else {
                    sb.append(cCharAt)
                }
                i3++
            }
        }

        fun getCallingActivity() {
            return this.mCallingActivity
        }

        fun getCallingActivityIcon() {
            if (this.mCallingActivity == null) {
                return null
            }
            try {
                return this.mActivity.getPackageManager().getActivityIcon(this.mCallingActivity)
            } catch (PackageManager.NameNotFoundException e) {
                Log.e(TAG, "Could not retrieve icon for calling activity", e)
                return null
            }
        }

        fun getCallingApplicationIcon() {
            if (this.mCallingPackage == null) {
                return null
            }
            try {
                return this.mActivity.getPackageManager().getApplicationIcon(this.mCallingPackage)
            } catch (PackageManager.NameNotFoundException e) {
                Log.e(TAG, "Could not retrieve icon for calling application", e)
                return null
            }
        }

        fun getCallingApplicationLabel() {
            if (this.mCallingPackage == null) {
                return null
            }
            PackageManager packageManager = this.mActivity.getPackageManager()
            try {
                return packageManager.getApplicationLabel(packageManager.getApplicationInfo(this.mCallingPackage, 0))
            } catch (PackageManager.NameNotFoundException e) {
                Log.e(TAG, "Could not retrieve label for calling application", e)
                return null
            }
        }

        fun getCallingPackage() {
            return this.mCallingPackage
        }

        public Array<String> getEmailBcc() {
            return this.mIntent.getStringArrayExtra("android.intent.extra.BCC")
        }

        public Array<String> getEmailCc() {
            return this.mIntent.getStringArrayExtra("android.intent.extra.CC")
        }

        public Array<String> getEmailTo() {
            return this.mIntent.getStringArrayExtra("android.intent.extra.EMAIL")
        }

        fun getHtmlText() {
            String stringExtra = this.mIntent.getStringExtra(IntentCompat.EXTRA_HTML_TEXT)
            if (stringExtra == null) {
                CharSequence text = getText()
                if (text is Spanned) {
                    return Html.toHtml((Spanned) text)
                }
                if (text != null) {
                    if (Build.VERSION.SDK_INT >= 16) {
                        return Html.escapeHtml(text)
                    }
                    StringBuilder sb = StringBuilder()
                    withinStyle(sb, text, 0, text.length())
                    return sb.toString()
                }
            }
            return stringExtra
        }

        fun getStream() {
            return (Uri) this.mIntent.getParcelableExtra("android.intent.extra.STREAM")
        }

        fun getStream(Int i) {
            if (this.mStreams == null && isMultipleShare()) {
                this.mStreams = this.mIntent.getParcelableArrayListExtra("android.intent.extra.STREAM")
            }
            if (this.mStreams != null) {
                return (Uri) this.mStreams.get(i)
            }
            if (i == 0) {
                return (Uri) this.mIntent.getParcelableExtra("android.intent.extra.STREAM")
            }
            throw IndexOutOfBoundsException("Stream items available: " + getStreamCount() + " index requested: " + i)
        }

        fun getStreamCount() {
            if (this.mStreams == null && isMultipleShare()) {
                this.mStreams = this.mIntent.getParcelableArrayListExtra("android.intent.extra.STREAM")
            }
            return this.mStreams != null ? this.mStreams.size() : this.mIntent.hasExtra("android.intent.extra.STREAM") ? 1 : 0
        }

        fun getSubject() {
            return this.mIntent.getStringExtra("android.intent.extra.SUBJECT")
        }

        fun getText() {
            return this.mIntent.getCharSequenceExtra("android.intent.extra.TEXT")
        }

        fun getType() {
            return this.mIntent.getType()
        }

        fun isMultipleShare() {
            return "android.intent.action.SEND_MULTIPLE".equals(this.mIntent.getAction())
        }

        fun isShareIntent() {
            String action = this.mIntent.getAction()
            return "android.intent.action.SEND".equals(action) || "android.intent.action.SEND_MULTIPLE".equals(action)
        }

        fun isSingleShare() {
            return "android.intent.action.SEND".equals(this.mIntent.getAction())
        }
    }

    private constructor() {
    }

    fun configureMenuItem(Menu menu, Int i, IntentBuilder intentBuilder) {
        MenuItem menuItemFindItem = menu.findItem(i)
        if (menuItemFindItem == null) {
            throw IllegalArgumentException("Could not find menu item with id " + i + " in the supplied menu")
        }
        configureMenuItem(menuItemFindItem, intentBuilder)
    }

    fun configureMenuItem(MenuItem menuItem, IntentBuilder intentBuilder) {
        ActionProvider actionProvider = menuItem.getActionProvider()
        ShareActionProvider shareActionProvider = !(actionProvider is ShareActionProvider) ? ShareActionProvider(intentBuilder.getActivity()) : (ShareActionProvider) actionProvider
        shareActionProvider.setShareHistoryFileName(HISTORY_FILENAME_PREFIX + intentBuilder.getActivity().getClass().getName())
        shareActionProvider.setShareIntent(intentBuilder.getIntent())
        menuItem.setActionProvider(shareActionProvider)
        if (Build.VERSION.SDK_INT >= 16 || menuItem.hasSubMenu()) {
            return
        }
        menuItem.setIntent(intentBuilder.createChooserIntent())
    }

    fun getCallingActivity(Activity activity) {
        ComponentName callingActivity = activity.getCallingActivity()
        return callingActivity == null ? (ComponentName) activity.getIntent().getParcelableExtra(EXTRA_CALLING_ACTIVITY) : callingActivity
    }

    fun getCallingPackage(Activity activity) {
        String callingPackage = activity.getCallingPackage()
        return callingPackage == null ? activity.getIntent().getStringExtra(EXTRA_CALLING_PACKAGE) : callingPackage
    }
}
