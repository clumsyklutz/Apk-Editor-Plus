# ApkEditorPlus

![Preview](https://raw.githubusercontent.com/FabioSilva11/Apk-Editor-Plus/master/APKEditor_Preview1.png)

## 📱 Sobre o Projeto

**ApkEditorPlus** é um projeto **open source** e **decompilado** de editor de APK para Android, desenvolvido em Kotlin. Este projeto foi decompilado e refatorado do código original em Java para uma versão mais moderna e atualizada.

## ⚠️ Status do Projeto

Este projeto ainda precisa de **várias correções**, mas as **funcionalidades estão presentes**, assim como as **interfaces e bibliotecas essenciais** para o funcionamento do aplicativo.

## 🔄 Migração Java → Kotlin

O projeto original era em **Java**, mas foi completamente refeito em **Kotlin** mais moderno e com **APIs mais atuais** do Android. A migração permite aproveitar:
- Sintaxe mais moderna e concisa
- APIs mais atuais do Android
- Melhor integração com as bibliotecas modernas do ecossistema Android
- Type safety e null safety nativos
- Coroutines para operações assíncronas

## 🛠️ Tecnologias Utilizadas

- **Kotlin** - Linguagem principal
- **Android SDK** - API 24+ (Android 7.0+)
- **Target SDK** - API 36
- **Gradle** - Sistema de build
- **AndroidX Libraries** - Bibliotecas modernas do Android

## 📋 Funcionalidades

O ApkEditorPlus oferece diversas funcionalidades para edição e manipulação de arquivos APK:

- Visualização e edição de informações do APK
- Edição de recursos (imagens, XML, etc.)
- Edição de código Smali
- Compilação e descompilação de APKs
- Gerenciamento de projetos
- Editor de imagens PNG
- Visualizador de banco de dados SQLite
- E muito mais...

## 🚀 Como Compilar

1. Clone o repositório:
```bash
git clone https://github.com/FabioSilva11/Apk-Editor-Plus.git
cd Apk-Editor-Plus
```

2. Abra o projeto no Android Studio

3. Sincronize as dependências do Gradle

4. Compile e execute o projeto

## 📝 Requisitos

- Android Studio (versão mais recente recomendada)
- JDK 11 ou superior
- Android SDK com API 24+
- Gradle

## 🤝 Contribuindo

Contribuições são bem-vindas! Este projeto está em desenvolvimento ativo e há muito espaço para melhorias. Sinta-se à vontade para:

- Reportar bugs
- Sugerir novas funcionalidades
- Enviar pull requests
- Melhorar a documentação

## ⚠️ Aviso Legal

Este projeto foi criado para fins educacionais e de pesquisa. Certifique-se de usar este software de forma ética e legal, respeitando os termos de serviço e direitos autorais dos aplicativos que você editar.

## 📄 Licença

Este projeto é open source. Consulte o arquivo LICENSE para mais detalhes.

## 👤 Autor

Projeto decompilado e refatorado para Kotlin.

---

**Nota**: Este projeto ainda está em desenvolvimento e pode conter bugs. Use por sua conta e risco.

